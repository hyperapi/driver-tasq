...
require("@kirick/tasq");
//#region src/main.ts
var HyperAPITasqDriver = class extends _hyperapi_core_dev.HyperAPIDriver {
	#server;
	/**
		...
		this.#server = tasq.serve({
			...
			handler: async (path, args) => {
				...
				try {
					...
					response[0] = false;
					if (error instanceof _hyperapi_core.HyperAPIError) response[1] = error.getResponse();
					else {
						console.error("Unhandled error in @hyperapi/driver-tasq:");
						console.error(error);
						response[1] = new _hyperapi_core.HyperAPIInternalError().getResponse();
					}
				}
				return response;
			}
	...
	*/
	async processRequest(path, args) {
		...
		if (hyperapi_response instanceof _hyperapi_core.HyperAPIError) throw hyperapi_response;
